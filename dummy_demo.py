...
# For now using the neural network that is provided. NN: 1 hidden layer, consisting of 5 hidden nodes.
hidden_nodes = 10
...
def population_initialisation(lower_bound_weights, upper_bound_weights, population_size, chromosome_length):
    return np.random.uniform(lower_bound_weights, upper_bound_weights,
                             (population_size, chromosome_length))  # use uniform distribution to sample
...
def parent_selection(population_size, mating_pool_size, pop, fit_pop, mating_tournament_sample_size):
    ...
    mating_pool, fit_mating_pool = tournament_selection(pop, fit_pop, population_size, mating_tournament_sample_size,
                                                        mating_pool_size)
...
def crossover(pop, population_size, recombination_probability, chromosome_length, mating_pool_size, fit_pop,
              mating_tournament_sample_size, recombination_scale):
    # single arithmetic recombination
    alpha = recombination_probability
    parents, fit_parents = parent_selection(population_size, mating_pool_size, pop, fit_pop,
                                            mating_tournament_sample_size)
...
for run in range(1, 11):
    ...
    env = Environment(experiment_name=experiment_name,
                      enemies=[enemy],
                      playermode="ai",
                      player_controller=player_controller(hidden_nodes),
                      enemymode="static",
                      level=2,
                      speed="fastest",
                      randomini='yes')
...
# Set necessary parameters
    ...
    bias = 1
    action_space = 5
    chromosome_length = (observation_space + bias) * hidden_nodes + (hidden_nodes + bias) * action_space
...
# loads file with the best solution for testing
    ...
    for i in range(1, generations):
        ...
        offspring, parents, fit_parents = crossover(pop, population_size, recombination_probability, chromosome_length,
                                                    mating_pool_size, fit_pop, mating_tournament_sample_size,
                                                    recombination_scale)
